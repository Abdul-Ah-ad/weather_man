        ...
        try:
            with open(filepath, 'r', encoding='utf-8-sig') as file:
                ...
                for row in reader:
                    ...
                    try:
                        max_temp = int(row[1]) if is_valid_temperature(row[1]) else None
                        min_temp = int(row[3]) if is_valid_temperature(row[3]) else None
                        max_humidity = int(row[7]) if is_valid_humidity(row[7]) else None
                        min_humidity = int(row[9]) if is_valid_humidity(row[9]) else None
                    except ValueError:
                        continue
